...
@triton.jit
def _sparse_indices_attn_res_fwd(
    Q, K, V, O, M, INDICES,
    PK, PV,
    ...
    for k_id in range(0, n_ctx // B_SIZE // BLOCK_N):
...
def sparse_indices_attn_res_fwd(q, k, v, pk, pv, indices,
                                topk, token_weight=1, down_scale=4):
    BLOCK_N = 64
    ...
    _sparse_indices_attn_res_fwd[grid2](
        q, k, v, o, m, indices,
        pk, pv,
        ...
        M, N, H, scale, topk,
        down_scale, token_weight, B_SIZE, BLOCK_N,
        D_HEAD=D,
    )
...
def _attn_bwd_res_dqkv(DQ, DK, DV, DPK, DPV,
    ...
    for k_id in range(n_ctx // BLOCK_M // BLOCK_M):
...
@triton.jit
def _attn_bwd_res_dpkv(DPK, DPV,
                       Q,
                       PK, PV,
                       DO, M, D,
                       stride_qb, stride_qh, stride_qm, stride_qd,
...
def sparse_indices_attn_res_scatter_bwd_v2(do, q, k, v, pk, pv, o, m, indices,
                                           topk,
                                           block_size, token_weight):
