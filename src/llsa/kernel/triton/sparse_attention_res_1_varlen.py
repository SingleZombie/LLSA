...
@triton.jit
def _sparse_indices_attn_res_varlen_fwd(
    Q, K, V, OUT, M, INDICES,
    PK, PV,
    ...
    stride_pvb, stride_pvh, stride_pvn, stride_pvd,
    m_ctx, n_ctx, p_ctx, NUM_HEAD: tl.constexpr,
    scale: tl.constexpr,
    TOPK: tl.constexpr,
    ...
    for k_id in range(0, (p_ctx + BLOCK_N - 1) // BLOCK_N):
        offsets_n_2 = k_id * BLOCK_N + tl.arange(0, BLOCK_N)
        mask_p = offsets_n_2 < p_ctx

        k_ptrs = (PK + cur_b * stride_pkb + cur_h * stride_pkh
                  + offsets_n_2[:, None] * stride_pkn
                  + offsets_d[None, :] * stride_pkd)
        v_ptrs = (PV + cur_b * stride_pvb + cur_h * stride_pvh
                  + offsets_n_2[:, None] * stride_pvn
                  + offsets_d[None, :] * stride_pvd)

        k = tl.load(k_ptrs, mask=mask_p[:, None], other=0.0)
        v = tl.load(v_ptrs, mask=mask_p[:, None], other=0.0)

        scores = tl.dot(q, tl.trans(k)).to(tl.float32)
        scores = scores + tl.where(mask_p[None, :], 0.0, -1.0e6)

        weights = tl.full([BLOCK_N], TOKEN_WEIGHT, tl.float32)
        is_last = offsets_n_2 == (p_ctx - 1)
        weights = tl.where((BLOCK_LAST_WEIGHT > 0) &
                           is_last, BLOCK_LAST_WEIGHT, weights)
        ...
        l_i = l_i * tl.exp(m_i - m_i_new) + l_ij
        acc = acc * tl.exp(m_i - m_i_new)[:, None] + \
            tl.dot(exp_scores_w.to(v.dtype), v)
        m_i = m_i_new

    acc /= l_i[:, None]
    o_ptrs = (OUT + cur_b * stride_qb + cur_h * stride_qh
              + offsets_m[:, None] * stride_qm
              + offsets_d[None, :] * stride_qd)
...
def sparse_indices_attn_res_varlen_fwd(q, k, v, pk, pv, indices,
                                       topk, token_weight=1, block_last_weight=0, down_scale=4):
    block_n = 64
    ...
    _sparse_indices_attn_res_varlen_fwd[grid](
        q, k, v, o, m, indices,
        pk, pv,
        ...
        m_ctx, n_ctx, p_ctx, n_head, scale, topk,
        down_scale, token_weight, block_last_weight, b_size, block_n,
        D_HEAD=d_head,
    )
...
@triton.jit
def _attn_bwd_preprocess(OUT, DO,
                         Delta,
    ...
    o = tl.load(OUT + cur_b * stride_ob + cur_h * stride_oh
    ...
    tl.store(Delta + cur_b * stride_delata_b + cur_h * stride_delta_h + off_m * stride_delta_m,
             delta, mask=mask_m)
...
def _attn_bwd_res_dqkv(
    ...
    H, n_ctx, p_ctx, scale, block_last_weight,
    TOPK: tl.constexpr,
    TOKEN_WEIGHT: tl.constexpr,
    BLOCK_M: tl.constexpr,
    ...
    for k_id in range(0, (p_ctx + BLOCK_M - 1) // BLOCK_M):
        offs_n_2 = k_id * BLOCK_M + tl.arange(0, BLOCK_M)
        mask_p = offs_n_2 < p_ctx

        kT_ptrs = PK + offs_n_2[None, :] * \
            stride_pkn + offs_k[:, None] * stride_pkd
        vT_ptrs = PV + offs_n_2[None, :] * \
            stride_pkn + offs_k[:, None] * stride_pkd
...
def _attn_bwd_res_dpkv(
    ...
    weights = tl.where((block_last_weight > 0) & is_last,
                       block_last_weight, weights)
    weights = tl.where(mask_p, weights, 0.0)
...
def sparse_indices_attn_res_bwd_v2_varlen(do, q, k, v, pk, pv, o, m, indices,
                                          topk, block_size, token_weight, block_last_weight):
    ...
    _attn_bwd_res_dqkv[grid](
        ...
        n_head, m_ctx, p_ctx, scale, block_last_weight,
        topk, token_weight,
        block_size, block_n,
        HEAD_DIM=head_dim,
    )
